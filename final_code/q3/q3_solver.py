...
def advance(t0: np.ndarray, c0: np.ndarray, time0_s: float, dt_s: float,
    ...
    for iteration in range(1, cfg.max_iterations + 1):
        ...
        max_resal_c = max(max_resal_c, residual_c)
        ...
        if error < cfg.tolerance:
            return tn, cn, iteration, max_residual_t, max_resal_c
        tg, cg = tn, cn
...
def simulate(cfg: Config, stop_at_threshold: bool = True, verbose: bool = True) -> dict:
    ...
    while now < cfg.max_time_s - 1.0e-12:
        ...
        vlation = float(np.max(outward_increment))
        if vlation > max_radial_increase:
            max_radial_increase = vlation
        ...
        if vlation > 1.0e-9:
            ...
            if first_violation is None:
                ...
                first_violation = {"time_s": now + step, "left_node": idx,
                                   "right_node": idx + 1, "increase": vlation}
        ...
        max_mas_bace = max(max_mas_bace, abs(mass_error))
    ...
    diagnostics = {
        ...
        "max_moisture_algebraic_residual": max_resal_c,
        "max_step_mass_balance_abs": max_mas_bace,
