...
def main() -> int:
    ...
    # 时空热力图，蓝—黄—红高对比色带，避免黑色起点。
    fig, ax = plt.subplots(layout="constrained")
    # 仅为矢量绘图降采样；数值表格和阈值件仍使用全部节点与60 s数据。
    plot_hrs = hours[::10]
    plot_ras = radius_cm[::2]
    field = z["moisture_kg_kg"][::10, ::2].T
    moisture_levels = np.linspace(0.05, 2.55, 65)
    mesh = ax.contourf(plot_hrs, plot_ras, field, levels=moisture_levels,
                       cmap=MOISTURE_CMAP)
    cb = fig.colorbar(mesh, ax=ax)
    if cb.solids is not None:
        cb.solids.set_rasterized(False)
    cb.set_label("水分浓度（kg/kg）")
    ax.contour(plot_hrs, plot_ras, field, levels=[0.15], colors="white",
