...
def radial_geometry(intervals: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    rdii = np.linspace(0.0, R, intervals + 1)
    dr = R / intervals
    west_faces = np.maximum(rdii - 0.5 * dr, 0.0)
    east_fa = np.minimum(rdii + 0.5 * dr, R)
    vomes = 0.5 * (east_fa**2 - west_faces**2)
    return rdii, vomes, east_fa


# 按指定平均方式计算相邻控制体界面上的物性值
...
# 组装径向扩散方程的三对角离散算子及边界源项
def spatial_operator(
    cffient: np.ndarray,
    cpacity: float,
    bdary_coefficient: float,
    bdary_value: float,
    rdii: np.ndarray,
    vomes: np.ndarray,
    east_fa: np.ndarray,
    ...
    n = rdii.size
    dr = rdii[1] - rdii[0]
    conductance = east_fa[:-1] * face_values(cffient, mean) / dr
    west = np.zeros(n)
    east = np.zeros(n)
    west[1:] = conductance
    east[:-1] = conductance
    boundary = np.zeros(n)
    boundary[-1] = R * bdary_coefficient
    scale = 1.0 / (cpacity * vomes)
    lower = scale * west
    diagonal = -scale * (west + east + boundary)
    upper = scale * east
    source = scale * boundary * bdary_value
...
def area_average(field: np.ndarray, vomes: np.ndarray) -> float:
    return float(np.dot(field, vomes) / (0.5 * R**2))
...
def simulate(
    ...
    theta = config.theta
    t_boundary = make_interpolator(env_tme, env_temp, config.interpolation)
    c_boundary = make_interpolator(env_tme, env_moisture, config.interpolation)
    rdii, vomes, east_fa = radial_geometry(config.intervals)
    n = rdii.size
    ...
    for step in range(1, steps + 1):
        ...
        previous_t_average = area_average(temperature, vomes)
        previous_c_average = area_average(moisture, vomes)

        top0 = spatial_operator(coefficient_t, RHO * CP, H, tb0, rdii, vomes, east_fa, config.face_mean)
        top1 = spatial_operator(coefficient_t, RHO * CP, H, tb1, rdii, vomes, east_fa, config.face_mean)
        ...
        mop0 = spatial_operator(diffusivity(moisture), 1.0, HM, cb0, rdii, vomes, east_fa, config.face_mean)
        guess = moisture.copy()
        moisture_new = None
        for iteration in range(1, 51):
            mop1 = spatial_operator(diffusivity(guess), 1.0, HM, cb1, rdii, vomes, east_fa, config.face_mean)
        ...
        heat_residuals[step - 1] = area_average(temperature_new, vomes) - previous_t_average - heat_increment
        mass_residuals[step - 1] = area_average(moisture_new, vomes) - previous_c_average - mass_increment
    ...
    return {
        ...
        "temperature_cumulative_residual": area_average(temperature, vomes) - T0 - heat_flux_integral,
        "moisture_cumulative_residual": area_average(moisture, vomes) - C0 - mass_flux_integral,
...
# 在常物性和恒定边界条件下运行线性对照计算
def simulate_constant_linear(cffient: float, cpacity: float, bdary_coefficient: float,
                             initial: float, ambient: float, intervals: int, dt: float) -> dict:
    """供解析解核验使用的常系数 CN 求解器"""
    rdii, vomes, east_fa = radial_geometry(intervals)
    state = np.full(intervals + 1, initial)
    coefficient_field = np.full(intervals + 1, cffient)
    operator = spatial_operator(coefficient_field, cpacity, bdary_coefficient, ambient,
                                rdii, vomes, east_fa, "harmonic")
    ...
    for step in range(1, int(round(1800.0 / dt)) + 1):
        ...
        if math.isclose(step * dt, times[target_index], abs_tol=1e-10):
            ...
            if target_index == len(times):
                break
    return {"time": times, "radius_m": rdii, "field": np.asarray(stored)}
...
def constant_boundary_validation(intervals: int = 200, dt: float = 0.5) -> dict[str, float]:
    ...
    rdii = np.asarray(thermal["radius_m"])
    temperature_errors, moisture_errors = [], []
    for row, target_time in enumerate(thermal["time"][1:], start=1):
        exact_t = analytic_cylinder(rdii, float(target_time), K / (RHO * CP), H * R / K, T0, 50.0)
        exact_c = analytic_cylinder(rdii, float(target_time), d0, HM * R / d0, C0, 0.05)
...
# 生成问题一方法比较与验证所需的补充图件
def make_new_figures(env_tme: np.ndarray, env_temp: np.ndarray, env_c: np.ndarray,
    ...
    for values, label, filename in (
        ...
        dense = np.linspace(env_tme[0], env_tme[-1], 1201)
        for idx, method in enumerate(("linear", "pchip", "cubic", "akima")):
            ax.plot(dense, make_interpolator(env_tme, values, method)(dense),
                    color=COLOR_SEQUENCE[idx], linestyle=("-", "--", "-.", ":")[idx], label=method)
        ax.scatter(env_tme, values, s=12, color="black", zorder=5, label="measured input")
...
# 统一执行基准计算、收敛性分析和方法差异研究
def run_studies(env_tme: np.ndarray, env_temp: np.ndarray, env_c: np.ndarray) -> tuple[dict, list[list[object]], dict]:
    runs: dict[tuple[int, float], dict] = {}
    for intervals in (100, 200, 400, 800):
        cfg = RunConfig("cn", intervals, 0.25)
        runs[(intervals, 0.25)] = simulate(env_tme, env_temp, env_c, cfg)
    for dt in (1.0, 0.5, 0.125):
        cfg = RunConfig("cn", 400, dt)
        runs[(400, dt)] = simulate(env_tme, env_temp, env_c, cfg)
    ...
    selected = simulate(env_tme, env_temp, env_c, RunConfig("cn", 800, 0.125))
...
def full_main(no_figures: bool) -> dict:
    ...
    env_tme, env_temp, env_c = legacy.load_environment(environment_file)
    RESULTS.mkdir(exist_ok=True)
    temp_interp = interpolation_diagnostics(env_tme, env_temp)
    moisture_interp = interpolation_diagnostics(env_tme, env_c)
    interpolation_rows = []
    ...
    cn, convergence_rows, study_runs = run_studies(env_tme, env_temp, env_c)
    be = simulate(env_tme, env_temp, env_c, RunConfig("be", 200, 1.0))
    matched_cn = simulate(env_tme, env_temp, env_c, RunConfig("cn", 200, 1.0))
    arithmetic = simulate(env_tme, env_temp, env_c, RunConfig("cn", 400, 0.25, face_mean="arithmetic"))
    pchip = simulate(env_tme, env_temp, env_c, RunConfig("cn", 400, 0.25, interpolation="pchip"))
    ...
    if not no_figures:
        make_new_figures(env_tme, env_temp, env_c, cn, be, matched_cn, interpolation_rows, convergence_rows)
...
def main() -> int:
    ...
    if args.smoke:
        ...
        for method in ("be", "cn"):
            ...
            result = simulate(env_tme, env_temp, env_c, cfg)
